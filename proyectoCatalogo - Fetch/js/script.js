...
function filtrarArticulos(array) {
    let articulo = selectPro.value;
    if (!articulo) {
        return array;
    } else {
        return array.filter((item) => item.rubroArticulo == articulo);
    }
}
        ...
        const tarjeta = `
            <div class="col">
                <div class="card h-100">
                    <img src="${articulo.foto}" class="card-img-top" alt="${articulo.codigo}">
                    <div class="card-body">
                        <h5 class="card-title">${articulo.codigo}</h5>
                        <p class="card-text">Nombre: ${articulo.nombre}</p>
                        <p class="card-text">Casa: ${articulo.precio}</p>
                    </div>
                </div>
            </div>`;
...
async function buscarArticulos() {
    ...
    crearHTML(filtrarArticulos(data));
}
